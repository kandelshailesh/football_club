...
export const createNotice = async (param) => {
  try {
    let ab = null;
    let params = param;
    console.log("parmss11", params);
    const [err, data] = await too(
      notice.findOne({ where: { title: params["title"] } })
    );
    ...
    if (ab == null) {
      ...
      if (params?.id) {
        ...
        const [err1, data1] = await to(notice.create({ ...params }));
        console.log("hiii 0", data1, err1);
        if (err1) TE(err1.message);
        if (!data1) TE("Error creating Notice");
        return data1;
      }
    }
...
export const getNotice = async (param) => {
  let page, limit;
...
export const deleteNotice = async (id) => {
  try {
    const [err, data] = await too(notice.destroy({ where: { id: id } }));
    if (err) TE(err.message);
